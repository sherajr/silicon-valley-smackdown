...
export class CombatSim {
  ...
  private applyBlockedHit(attacker: FighterRuntime, defender: FighterRuntime, effect: HitEffect, events: SimEvent[], attackerSlot: PlayerSlot, defenderSlot: PlayerSlot): void {
    // Captured before defender.state changes below -- the presentation layer needs to know
    // whether this was a standing or crouching guard, since by the time it sees the event,
    // defender.state has already moved on to 'blockstun'/'guardbreak'.
    const crouching = defender.state === 'crouch';
