...
export class ResultsScene extends Phaser.Scene {
  ...
  private createArcadeResults(data: FightSceneResult): void {
    ...
    this.add
      .text(BASE_WIDTH / 2, 60, playerWon ? 'VICTORY!' : 'DEFEATED', { fontFamily: 'monospace', fontSize: '22px', color: playerWon ? '#ffd23f' : '#ff6b6b' })
      .setOrigin(0.5, 0.5);
    this.add
      .text(BASE_WIDTH / 2, 90, `${data.scoreP1} - ${data.scoreP2}  vs  ${opponentDef.name}`, { fontFamily: 'monospace', fontSize: '10px', color: '#c8c8d8' })
      .setOrigin(0.5, 0.5);

    if (playerWon) {
      this.add.text(BASE_WIDTH / 2, 115, `"${opponentDef.winLine}" -- well, not this time.`, { fontFamily: 'monospace', fontSize: '8px', color: '#8a8a99' }).setOrigin(0.5, 0.5);
  ...
  private createVersusResults(data: FightSceneResult): void {
    ...
    this.add.text(BASE_WIDTH / 2, 50, `${data.matchWinner.toUpperCase()} WINS THE MATCH`, { fontFamily: 'monospace', fontSize: '16px', color: '#ffd23f' }).setOrigin(0.5, 0.5);
    this.add
