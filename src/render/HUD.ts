...
import { formatRoundClock } from '../sim/roundClock';
import { BASE_WIDTH, ROUND_TIME_FRAMES, MAX_GUARD, MAX_HYPE, ROUNDS_TO_WIN } from '../sim/constants';
...
export class HUD {
  ...
  constructor(scene: Phaser.Scene, p1Def: CharacterDef, p2Def: CharacterDef, p1Label: string, p2Label: string) {
    ...
    this.timerText = scene.add.text(BASE_WIDTH / 2, 12, formatRoundClock(ROUND_TIME_FRAMES), { fontFamily: 'monospace', fontSize: '16px', color: '#ffffff' }).setOrigin(0.5, 0);
    this.calloutText = scene.add
      .text(BASE_WIDTH / 2, 60, '', { fontFamily: 'monospace', fontSize: '12px', color: '#fff23d' })
      .setOrigin(0.5, 0.5)
      .setAlpha(0);
  ...
  private buildSide(x: number, name: string, label: string, rightAligned: boolean): SideWidgets {
    const container = this.scene.add.container(x, 26);
    ...
    const bar = this.scene.add.rectangle(0, 0, BAR_W, 8, rightAligned ? 0xff36c8 : 0x00e5ff).setOrigin(barOriginX, 0);
    const guard = this.scene.add.rectangle(0, 10, BAR_W * 0.7, 3, 0x00e5ff).setOrigin(barOriginX, 0);
    const hype = this.scene.add.rectangle(0, 15, 0, 3, 0xff36c8).setOrigin(barOriginX, 0);
    const hypeLabel = this.scene.add.text(0, 19, '', { fontFamily: 'monospace', fontSize: '7px', color: '#ff8de0' }).setOrigin(rightAligned ? 1 : 0, 0);
    ...
    const comboText = this.scene.add.text(0, 34, '', { fontFamily: 'monospace', fontSize: '9px', color: '#fff23d' }).setOrigin(rightAligned ? 1 : 0, 0);
  ...
  private updateSide(w: SideWidgets, f: FighterRuntime, wins: number): void {
    ...
    w.hypeBar.fillColor = hypePct >= 1 ? 0xffe36e : 0xff36c8;
    w.hypeLabel.setText(hypePct >= 1 ? 'HYPE READY' : '');
    for (let i = 0; i < w.roundPips.length; i++) {
      w.roundPips[i].setFillStyle(i < wins ? 0xfff23d : 0x3a3a44);
    }
    const buffs: string[] = [];
  ...
  update(p1: FighterRuntime, p2: FighterRuntime, scoreP1: number, scoreP2: number, clockFrames: number): void {
    ...
    this.timerText.setText(formatRoundClock(clockFrames));
    this.timerText.setColor(clockFrames <= 600 ? '#ff36c8' : '#fff23d');
  }
